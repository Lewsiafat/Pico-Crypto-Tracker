# Track Plan: 建立核心顯示與 Wi-Fi 連線功能

## Phase 1: 基礎硬體與網路環境設定 [checkpoint: 0c30154]
- [x] Task: 初始化 Pico Display Pack 2.8 並顯示啟動畫面 (Splash Screen) 8465892
- [x] Task: 實作 Wi-Fi 連線公用程式，並在螢幕上顯示連線狀態圖示 f3f9479
- [x] Task: Conductor - User Manual Verification '基礎硬體與網路環境設定' (Protocol in workflow.md) 0c30154

## Phase 2: API 整合與價格顯示
- [ ] Task: 實作 API 模組，透過 Wi-Fi 獲取 BTC 即時價格
- [ ] Task: 設計並實作「現代暗黑模式」價格顯示頁面 (顯示 BTC 價格與 24h 變化)
- [ ] Task: Conductor - User Manual Verification 'API 整合與價格顯示' (Protocol in workflow.md)
